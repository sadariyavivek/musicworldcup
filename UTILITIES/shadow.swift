...
class ShadowView: UIView {
    ...
    @IBInspectable var cornerRadiuss: CGFloat = 5.0 {
        didSet {
    ...
    @IBInspectable var shadowOffsets: CGSize = CGSize(width: 0.0, height: 2) {
        didSet {
            self.updateProperties()
        }
    }
    /// The shadow radius of the `ShadowView`, inspectable in Interface Builder
    @IBInspectable var shadowRadiuss: CGFloat = 4.0 {
        didSet {
            self.updateProperties()
        }
    }
